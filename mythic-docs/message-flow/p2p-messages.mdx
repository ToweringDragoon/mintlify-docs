...
This "delegates" key sits at the same level as the "action" key and can go with any message (checkin, post\_response, get\_tasking). The "message" field is the checkin message from Agent2, the "uuid" field is the tempUUID that agent1 generated, and the "c2\_profile" is the name of the C2 profile that the two agents are using to connect.
    ...
    "delegates": [
        {
            ...
            "mythic_uuid": "optionally, a new uuid that Mythic knows Agent2 by",
            "new_uuid": "optionally, same data as mythic_uuid, just here so you don't have to have 'mythic' in the key field"
        }
    ]
}
```
