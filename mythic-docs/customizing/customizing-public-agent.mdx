...
By default, the local folder in Mythic/InstalledServices is mounted into the container at `/Mythic`. This means that when local things change (such as uploading a file into a container), it goes on disk where the service is installed.
