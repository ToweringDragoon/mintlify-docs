...
* `opsec_pre/post_bypass_role` - this determines who should be able to bypass this check. The default is `lead` to indicate that only the lead of the operation should be able to bypass it, but you can set it to `operator` to allow any operators to bypass the check. You can also set this to `other_operator` to indicate that somebody *other* than the operator that issued the task must approve it. This is helpful in cases where it's not necessarily a "block", but something you want to make sure operators acknowledge as a potential security risk
...
From the `opsec_pre` and `opsec_post` functions, you have access to the entire task/callback information like you do in [Create\_Tasking](/version-2.3/customizing/payload-type-development/create_tasking#available-context). Additionally, you have access to the entire RPC suite just like in [Create\_Tasking](/version-2.3/customizing/payload-type-development/create_tasking#rpc-functionality).
