...
description: "Payload Type information must be set and pulled from a definition either in Python or in GoLang."
---

Below is a python example of the `apfell` agent:
...
<Frame>
    <img src="/images/version-2.3/image-100.avif"/>
</Frame>
...
* `self.agent_code_path` - a `pathlib.Path` object pointing to the path of the `agent_code` directory that holds all the code for your payload. This is something you pre-define as part of your agent definition.
  ...
  * To access "test.js" in that "agent\_code" folder, simply do: `f = open(self.agent_code_path / "test.js", 'r')`.
...
* * the dictionary returned by `self.c2info[0].get_c2profile()` contains the following:
  ...
  * the dictionary returned by `self.c2info[0].get_parameters_dict()`contains the following:
    ...
    * where each `key` is the `key` value defined for the c2 profile's parameters and `value` is what the user supplied. You might be wondering where to get these keys? Well, it's not too crazy and you can view them right in the UI - [Name Fields](https://github.com/MythicMeta/Documentation/blob/primary/customizing/payload-type-development/broken-reference/README.md).
