...
Since there's so much variability possible within a C2 Docker container, there's some required structure and python files similar to how Payload Types are structured. This is covered in [C2 Profile Code](/version-2.3/customizing/c2-related-development/c2-profile-code).
...
<Info>
The C2 Profile has nothing to do with the *content* of the messages that are being sent. It has no influence on the encryption or what format the agent messages are in (JSON, binary, stego, etc). If you want to control that level of granularity, you need to check out the [Translation Containers](/version-2.3/customizing/payload-type-development/translation-containers).
</Info>
