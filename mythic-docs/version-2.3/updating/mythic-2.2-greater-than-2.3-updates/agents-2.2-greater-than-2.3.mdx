...
To update for Mythic version 2.3.7, you need to have the `mythic_payloadtype_container==0.1.7` PyPi version installed. This will report to Mythic as container version 12. After that, you'll need to perform the following updates so that your agent can successfully sync with Mythic and appear in the user interface. If you're using one of the `itsafeaturemythic` Docker images, check [this page](/version-2.3/customizing/payload-type-development/container-syncing#current-payloadtype-versions) for the appropriate Docker image to use.
...
More information on how to code browser scripts for the new UI can be found here: [What is Browser Scripting?](/version-2.3/customizing/payload-type-development/browser-scripting#what-is-browser-scripting).
