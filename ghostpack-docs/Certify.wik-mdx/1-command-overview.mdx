...
description: 'Complete reference guide to Certify commands, parameters, and usage examples for AD CS enumeration and exploitation.'
icon: 'terminal'
...
```bash Specific CA
Certify.exe enum-cas --ca "SERVER\CA-NAME"
```
</CodeGroup>
...
| `--subject <dn>` | Subject Name (SN) for the certificate request.<br/>Default: *Current User*. |
...
| `--machine` | Request the certificate as `SYSTEM` (the current machine).<br/>Default: *Current User*. |
| `--output-pem` | Output the certificate in the original PEM format.<br/>Default: *PFX*. |
...
| `--agent-pass <password>` | Password for the request agent certificate<br/>Default: *Empty*. |
...
| `--cert-pass <password>` | Password for the certificate.<br/>Default: *Empty*. |
| `--machine` | Request the certificate as `SYSTEM` (the current machine).<br/>Default: *Current User*. |
| `--output-pem` | Output the certificate in the original PEM format.<br/>Default: *PFX*. |
| `--install` | Install the certificate in the local certificate store. |
...
| `--ca-pass <password>` | Password for the CA signing certificate.<br/>Default: *Empty*. |
| `--output-path <path>` | File path to output the forged certificate.<br/>Default: *Outputs to Console*. |
| `--output-pass <password>` | Password for the output certificate.<br/>Default: *Empty*. |
