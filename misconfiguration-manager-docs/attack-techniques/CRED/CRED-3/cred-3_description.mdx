---
title: 'CRED-3'
description: 'Dump currently deployed secrets via WMI'
...
This process is automated in [SharpDPAPI](https://github.com/GhostPack/SharpDPAPI?tab=readme-ov-file#sccm) and [SharpSCCM](/sharpsccm-docs/commands/local/secrets).
...
The SharpSCCM [`local secrets -m wmi`](/sharpsccm-docs/commands/local/secrets#examples) command extends this technique to retrieve collection variables and task sequences via WMI, which may also contain secrets such as credentials.
